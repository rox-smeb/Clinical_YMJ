...
//  Modifyed by AnYanbo on 2016-7-29
//
...
- (BOOL)nxEV_hasRowsToDisplay;
{
    ...
    {
        NSUInteger count = [self numberOfRowsInSection:sectionIndex];
        
        /**
         *  Modifyed by AnYanbo
         *  列表中section里的cell数量为空 -> 检测是否包含section header 或者 footer
         */
        if (count == 0)
        {
            CGRect headerRect = [self rectForHeaderInSection:sectionIndex];
            CGRect footerRect = [self rectForFooterInSection:sectionIndex];

            // 如果cell数量为0 -> 存在header || footer
            if (headerRect.size.height != 0 || footerRect.size.height != 0)
            {
                count = 1;
            }
        }
