...
{
    objc_setAssociatedObject(self, "oldTitle", oldTitle, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (void)setChangedTitle:(NSString *)changedTitle
{
    objc_setAssociatedObject(self, "changedTitle", changedTitle, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (NSString*)oldTitle
{
    return objc_getAssociatedObject(self, "oldTitle");
}

- (NSString*)changedTitle
{
    return objc_getAssociatedObject(self, "changedTitle");
}
...
- (void)resetTitle
{
    NSString* currentTitle = self.visibleViewController.navigationItem.title;
    
    // Not oldTitle && not changed title. Don`t rest title
    if ([currentTitle isEqualToString:self.oldTitle]     == NO &&
        [currentTitle isEqualToString:self.changedTitle] == NO)
    {
        
    }
    else if ([currentTitle isEqualToString:self.changedTitle])          // Only is changedTitle. Reset title to oldTitle
    {
        self.visibleViewController.navigationItem.title = self.oldTitle;
    }
...
- (void)changeSGProgressWithTitle:(NSString *)title
{
    ...
    {
        self.oldTitle = self.visibleViewController.navigationItem.title;
    }
