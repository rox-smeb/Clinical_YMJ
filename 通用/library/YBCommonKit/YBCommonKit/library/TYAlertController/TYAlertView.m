...
@property (nonatomic, copy) BOOL (^handler)(TYAlertAction *);

@end

@implementation TYAlertAction

+ (instancetype)actionWithTitle:(NSString *)title style:(TYAlertActionStyle)style handler:(BOOL (^)(TYAlertAction *))handler
{
    return [[self alloc]initWithTitle:title style:style handler:handler];
}

- (instancetype)initWithTitle:(NSString *)title style:(TYAlertActionStyle)style handler:(BOOL (^)(TYAlertAction *))handler
{
    if (self = [super init]) {
        _title = title;
        _style = style;
...
- (void)configureProperty
{
    ...
    _textFieldorderWidth = KTextFeildBorderWidth;
...
- (void)addContentViews
{
    ...
    _textFieldContentView = textFeildContentView;
...
- (void)addTextFieldWithConfigurationHandler:(void (^)(UITextField *textFeild))configurationHandler
{
    ...
    textField.tag = kTextFeildOffset + _textFields.count;
...
- (UITextField*)textFieldWithTag:(NSInteger)tag
{
    ...
    {
        if ([textField isKindOfClass:[UITextField class]] && textField.tag == tag)
        {
            return textField;
        }
    }
...
{
    UITextField *textFeild = _textFields.lastObject;
    ...
    if (_textFields.count == 1) {
        ...
        [_textFieldContentView addConstraintToView:textFeild edageInset:UIEdgeInsetsMake(_textFieldorderWidth, _textFieldEdge, -_textFieldorderWidth, -_textFieldEdge)];
        [textFeild addConstarintWidth:0 height:_textFieldHeight];
    }else {
        // textFeild
        UITextField *lastSecondTextFeild = _textFields[_textFields.count - 2];
        [_textFieldContentView removeConstraintWithView:lastSecondTextFeild attribte:NSLayoutAttributeBottom];
        [_textFieldContentView addConstarintWithTopView:lastSecondTextFeild toBottomView:textFeild constarint:_textFieldorderWidth];
        [_textFieldContentView addConstarintWithView:textFeild topView:nil leftView:_textFieldContentView bottomView:_textFieldContentView rightView:_textFieldContentView edageInset:UIEdgeInsetsMake(0, _textFieldEdge, -_textFieldorderWidth, -_textFieldEdge)];
        [_textFieldContentView addConstarintEqualWithView:textFeild widthToView:nil heightToView:lastSecondTextFeild];
        ...
        [_textFieldContentView addConstarintWithTopView:separateView toBottomView:textFeild constarint:0];
