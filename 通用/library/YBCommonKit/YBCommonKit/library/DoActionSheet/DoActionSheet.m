...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _doButtonBorderWidth      = 1.0f;
        _doCancelBorderWidth      = 1.0f;
        _doDestructiveBorderWidth = 1.0f;
    }
    return self;
}
...
- (void)setButtonAttributes:(UIButton *)bt cancel:(BOOL)bCancel
{
    ...
    if (bCancel)
    {
        ...
        if (self.doCancelBorderColor != nil)
        {
            bt.layer.borderColor = self.doCancelBorderColor.CGColor;
            bt.layer.borderWidth = self.doCancelBorderWidth;
        }
    }
    else
    {
        ...
        if (self.doButtonBorderColor != nil)
        {
            bt.layer.borderColor = self.doButtonBorderColor.CGColor;
            bt.layer.borderWidth = self.doButtonBorderWidth;
        }
    }
...
- (void)showActionSheet
{
    ...
    if (_strTitle != nil && _strTitle.length > 0)
    {
        ...
        // underline
        UIView *vLine = [[UIView alloc] initWithFrame:CGRectMake(lbTitle.frame.origin.x, lbTitle.frame.origin.y + lbTitle.frame.size.height, lbTitle.frame.size.width, 0.5)];
    ...
    for (NSString *str in _aButtons)
    {
        ...
        if (nTagIndex == _nDestructiveIndex)
        {
            ...
            if (self.doDestructiveBorderColor != nil)
            {
                bt.layer.borderColor = self.doDestructiveBorderColor.CGColor;
                bt.layer.borderWidth = self.doDestructiveBorderWidth;
            }
        }
...
{
    [self hideAnimationWithIndex:sender.tag];
}
...
- (void)hideAnimationWithIndex:(NSInteger)index
{
...
- (void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event
{
    ...
    [self hideAnimationWithIndex:DO_AS_CANCEL_TAG];
}
