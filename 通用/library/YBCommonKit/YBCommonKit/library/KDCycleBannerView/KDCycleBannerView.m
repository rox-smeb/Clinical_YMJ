...
@property (assign, nonatomic) NSInteger chinaImageIndex;
...
- (void)loadData
{
    ...
    BOOL showPageControl = YES;
    
    _pageControl.numberOfPages = _datasourceImages.count;
    _pageControl.currentPage = 0;
    
    if (_datasourceImages.count == 1 && [_delegate respondsToSelector:@selector(showPageControlWhenOnlyOnePage)])
    {
        showPageControl = [_delegate showPageControlWhenOnlyOnePage];
    }
    if ([_delegate respondsToSelector:@selector(configPageControl:)])
    {
        [_delegate configPageControl:_pageControl];
    }
    
    [_pageControl setHidden:!showPageControl];
    ...
    for (NSInteger i = 0; i < _datasourceImages.count; i++)
    {
        ...
        imageView.backgroundColor = [UIColor clearColor];
        imageView.contentMode = [_datasource contentModeForImage:imageView atIndex:i];
        ...
        _chinaImageIndex = 0;
        _chinaImageIndex = [_datasource chinaImageViewWithIndex];
        if (_chinaImageIndex == 1) {
            UIImageView *chinaImage = [[UIImageView alloc] initWithFrame:CGRectMake(imageView.frame.origin.x+imageView.frame.size.width-60, imageView.frame.origin.y+10, 60, 18)];
            chinaImage.image = [UIImage imageNamed:@"hzzx_05@x3"];
            [imageView addSubview:chinaImage];
        }else if (_chinaImageIndex == 2){
            UIImageView *chinaImage = [[UIImageView alloc] initWithFrame:CGRectMake(imageView.frame.origin.x+imageView.frame.size.width-60, imageView.frame.origin.y+10, 60, 18)];
            chinaImage.image = [UIImage imageNamed:@"hzzx_04@x3"];
            [imageView addSubview:chinaImage];
        }
        [_scrollView addSubview:imageView];
    }
