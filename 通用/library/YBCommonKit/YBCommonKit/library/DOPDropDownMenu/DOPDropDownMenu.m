...
@property (nonatomic, assign) NSInteger currentSelectedItem;                // 当前选中的二级列表项
...
#define kButtomImageViewHeight          20
...
- (void)setTile:(NSString*)title column:(NSInteger)column
{
    if (column < _titles.count)
    {
        ...
        CGSize size = [self calculateTitleSizeWithString:titleLayer.string];
...
- (void)selectIndexPath:(DOPIndexPath *)indexPath
{
    ...
    if ([_dataSource menu:self numberOfRowsInColumn:indexPath.column] <= indexPath.row)
    {
        return;
    }
    ...
    {
        if ([_dataSource respondsToSelector:@selector(menu:titleForItemsInRowAtIndexPath:)])
        {
...
- (instancetype)initWithOrigin:(CGPoint)origin andHeight:(CGFloat)height
{
    ...
    if (self)
    {
        ...
        _dropMenuHeight = kTableViewHeight;
        ...
        [_leftTableView removeSeperatorBlank];
        [_leftTableView removeRedundanceSeperator];
        ...
        [_rightTableView removeSeperatorBlank];
        [_rightTableView removeRedundanceSeperator];
        _rightTableView.dataSource = self;
        _rightTableView.delegate = self;
        _rightTableView.separatorColor = kSeparatorColor;

        UIImage* image = [UIImage imageNamed:@"DOPDropDownMenu.bundle/icon_chose_bottom"];
    ...
    if (show) {
        ...
        [UIView animateWithDuration:0.2 animations:^{
            ...
            if (haveItems)
            {
                ...
                NSIndexPath* indexPath = [NSIndexPath indexPathForRow:currentSelectedMenudRow inSection:0];
                if (indexPath != nil)
                {
                    [_leftTableView scrollToRowAtIndexPath:indexPath atScrollPosition:UITableViewScrollPositionNone animated:NO];
                }
                
                if (_dataSourceFlags.titleForItemsInRowAtIndexPath)
                {
                    NSInteger count = [_dataSource menu:self numberOfItemsInRow:currentSelectedMenudRow column:_currentSelectedMenudIndex];
                    
                    for (NSInteger idx = 0; idx < count; idx++)
                    {
                        NSString* title = [_dataSource menu:self titleForItemsInRowAtIndexPath:[DOPIndexPath indexPathWithCol:_currentSelectedMenudIndex row:currentSelectedMenudRow item:idx]];
                        NSString* selectTitle = [(CATextLayer *)[_titles objectAtIndex:_currentSelectedMenudIndex] string];
                        if ([title isEqualToString:selectTitle])
                        {
                            NSIndexPath* indexPath = [NSIndexPath indexPathForRow:idx inSection:0];
                            if (indexPath != nil)
                            {
                                [_rightTableView scrollToRowAtIndexPath:indexPath atScrollPosition:UITableViewScrollPositionNone animated:NO];
                            }
                            break;
                        }
                    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (tableView == _leftTableView)
    {
        ...
        {
            UIImage* image1 = [UIImage imageNamed:@"DOPDropDownMenu.bundle/icon_chose_arrow_nor"];
            UIImage* image2 = [UIImage imageNamed:@"DOPDropDownMenu.bundle/icon_chose_arrow_sel"];
            cell.accessoryView = [[UIImageView alloc]initWithImage:image1 highlightedImage:image2];
        }
        else
        {
    ...
    else
    {
        ...
        if (self.currentSelectedItem == indexPath.row)
        {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if (_leftTableView == tableView)
    {
        ...
        {
            [self.delegate menu:self didSelectRowAtIndexPath:[DOPIndexPath indexPathWithCol:_currentSelectedMenudIndex row:indexPath.row item:self.currentSelectedItem]];
        }
    }
    else
    {
        self.currentSelectedItem = indexPath.item;
        [self configMenuWithSelectItem:indexPath.item];
...
- (void)configMenuWithSelectItem:(NSInteger)item
{
    CATextLayer *title = (CATextLayer *)_titles[_currentSelectedMenudIndex];
    NSInteger currentSelectedMenudRow = [_currentSelectRowArray[_currentSelectedMenudIndex] integerValue];
