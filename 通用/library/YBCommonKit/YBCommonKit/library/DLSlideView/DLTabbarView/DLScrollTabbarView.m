...
- (void)setTabbarItems:(NSArray *)tabbarItems{
    if (_tabbarItems != tabbarItems) {
        ...
        for (DLScrollTabbarItem *item in tabbarItems)
        {
            ...
            {
                if (item != [tabbarItems lastObject])
                {
                    if (item.separatorWidth == 0.0f)
                    {
                    ...
                    UIView* separtor = [[UIView alloc] initWithFrame:CGRectMake(item.width,
                                                                                item.separatorInsets.top,
                                                                                item.separatorWidth,
                                                                                height - item.separatorInsets.top - item.separatorInsets.bottom)];
                    separtor.backgroundColor = item.separatorColor;
                    [backView addSubview:separtor];
                }
            }
        ...
        scrollView_.bounces = self.scrollBounces;
        ...
        if (self.showSeparator)
        {
            ...
            {
                self.separatorWidth = kTrackViewHeight;
            }
            if (self.separatorColor == nil)
            {
                self.separatorColor = UIColorMake(220, 220, 220);
            }
            
            CGFloat offset = 0.0f;
            if (scrollView_.bounces)
            {
                offset = 1000.0f;
            }
            
            UIView* separator = [[UIView alloc] initWithFrame:CGRectMake(-offset,
                                                                         self.bounds.size.height - self.separatorWidth - 1,
                                                                         scrollView_.contentSize.width + offset * 2,
                                                                         self.separatorWidth)];
            separator.backgroundColor = self.separatorColor;
            
            [scrollView_ addSubview:separator];
            [scrollView_ bringSubviewToFront:trackView_];
        }
