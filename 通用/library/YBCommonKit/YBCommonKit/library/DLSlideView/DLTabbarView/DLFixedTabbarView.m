...
- (void)commonInit{
    ...
    _separtorView = [[UIView alloc] initWithFrame:CGRectMake(0, self.bounds.size.height - 2, self.bounds.size.width, 1.0f)];
    _separtorView.hidden = YES;
    [self addSubview:_separtorView];
...
//        assert(tabbarItems.count <= 4);
        ...
        if (self.showSeparator)
        {
            ...
            CGFloat offset = self.separatorInsets.left + self.separatorInsets.right;
            for (int i = 1; i < [tabbarItems count]; i++)
            {
                UIView* separator = [[UIView alloc] initWithFrame:CGRectMake(i * width + offset, self.separatorInsets.top, self.separatorWidth, height - self.separatorInsets.bottom - self.separatorInsets.top)];
                separator.backgroundColor = self.separatorColor;
                separator.tag = kSeparatorTagBase + i;
                [scrollView_ addSubview:separator];
            }
            
            _separtorView.hidden = NO;
            _separtorView.backgroundColor = self.separatorColor;
        }
...
- (void)layoutTabbar{
    ...
    {
        CGFloat offset = self.separatorInsets.left + self.separatorInsets.right;
        for (int i = 1; i < [self.tabbarItems count]; i++)
        {
            UIView* separator = (UIView *)[scrollView_ viewWithTag:kSeparatorTagBase + i];
            separator.frame = CGRectMake(i * width + offset, self.separatorInsets.top, self.separatorWidth, height - self.separatorInsets.bottom - self.separatorInsets.top);
        }
        
        _separtorView.frame = CGRectMake(0, self.bounds.size.height - self.separatorWidth * 2, self.bounds.size.width, self.separatorWidth);
    }
...
- (void)tapAction:(UITapGestureRecognizer *)tap{
    ...
    if ([self.delegate respondsToSelector:@selector(DLSlideTabbar:selectAt:)])
    {
        if ([self.delegate DLSlideTabbar:self selectAt:i])
        {
