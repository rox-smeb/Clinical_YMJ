...
#define kDefaultTabbarHeight 40.0f
...
- (void)commonInit{
    ...
    slideView_.transitionDur = self.transitionDur;
    slideView_.transitonOptions = self.transitonOptions;
...
- (void)buildTabbar{
    ...
    tabbar_.showSeparator = self.showSeparator;
    tabbar_.separatorColor = self.separatorColor;
    tabbar_.separatorInsets = self.separatorInsets;
    tabbar_.separatorWidth = self.separatorWidth;
...
- (BOOL)DLSlideTabbar:(id)sender selectAt:(NSInteger)index
{
    if ([slideView_ isSwitching])
    {
        return NO;
    }
