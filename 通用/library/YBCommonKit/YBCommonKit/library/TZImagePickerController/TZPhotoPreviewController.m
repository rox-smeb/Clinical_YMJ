...
- (void)configBottomToolBar {
    ...
    [_numberImageView addNormalTapGestureWithTarget:self atction:@selector(okButtonClick)];
...
- (void)select:(UIButton *)selectButton
{
    ...
    if (!selectButton.isSelected)
    {
        ...
        {
            if (imagePickerVc.allowPickingMoreWhenExceedMax)            // 取消之前第一个选择
            {
                TZAssetModel* firstModel = [self.selectedPhotoArr firstObject];
                firstModel.isSelected = NO;
                [self.selectedPhotoArr removeFirstObject];
            }
            else
            {
