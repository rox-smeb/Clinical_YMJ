...
- (void)configBottomToolBar {
    ...
    _numberLable.userInteractionEnabled = YES;
    [_numberLable addNormalTapGestureWithTarget:self atction:@selector(okButtonClick)];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.didSelectPhotoBlock = ^(BOOL isSelected) {
        ...
        else
        {
            ...
            {
                if (imagePickerVc.allowPickingMoreWhenExceedMax)        // 超过重置第一个选择
                {
                    // 取消之前的选择
                    TZAssetModel* firstModel = [weakSelf.selectedPhotoArr firstObject];
                    firstModel.isSelected = NO;
                    [weakSelf.selectedPhotoArr removeFirstObject];
                    ...
                    // 找到需要取消选择的那个IndexPath
                    NSInteger index = [_photoArr indexOfObject:firstModel];
                    NSIndexPath* reloadIndexPath = [NSIndexPath indexPathForItem:index inSection:0];
