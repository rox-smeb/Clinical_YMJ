...
- (CGFloat)heightForColumn:(NSInteger)column inSection:(NSInteger)section
{
    ...
    CGFloat space = self.interItemSpacingY;
    id delegate = self.collectionView.delegate;
    if ([delegate respondsToSelector:@selector(collectionView:layout:interItemSpacingInSection:)])
    {
        space = [delegate collectionView:self.collectionView layout:self interItemSpacingInSection:section];
    }
	return CGRectGetMaxY(attributes.frame) + space;
}
...
- (UICollectionViewLayoutAttributes *)layoutAttributesForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([delegate respondsToSelector:@selector(collectionView:layout:interItemSpacingInSection:)])
    {
        space = [delegate collectionView:self.collectionView layout:self interItemSpacingInSection:indexPath.section];
    }
    
	if (top == space) // We are the first cell
	{
        top -= space; // No spaceing on first cell in column
		...
		if ([self.headers objectForKey:sectionIndexPath])
		{
			...
			{
				top += headerAttributes.frame.size.height + space;
			}
		}
	}
