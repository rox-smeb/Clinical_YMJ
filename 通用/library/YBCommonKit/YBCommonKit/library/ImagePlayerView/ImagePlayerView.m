...
- (void)_init
{
    ...
    if ([self.imagePlayerViewDelegate respondsToSelector:@selector(imagePlayerView:scrollAtIndex:)])
    {
        [self.imagePlayerViewDelegate imagePlayerView:self scrollAtIndex:0];
    }
...
- (void)reloadData
{
    ...
    for (int i = 0; i < self.count; i++)
    {
        ...
        imageView.clipsToBounds = YES;                              // Modify by AnYanbo
...
- (void)handleScrollTimer:(NSTimer *)timer
{
    ...
    if ([self.imagePlayerViewDelegate respondsToSelector:@selector(imagePlayerView:scrollAtIndex:)])
    {
        [self.imagePlayerViewDelegate imagePlayerView:self scrollAtIndex:nextPage];
    }
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    if (_lastScrollIndex != currentIndex)
    {
        if ([self.imagePlayerViewDelegate respondsToSelector:@selector(imagePlayerView:scrollAtIndex:)])
        {
            [self.imagePlayerViewDelegate imagePlayerView:self scrollAtIndex:currentIndex];
        }
